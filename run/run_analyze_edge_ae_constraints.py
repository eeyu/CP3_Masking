...
def reconstruct_from_vae(model, masked_topologies, device='cpu'):
    with torch.no_grad():
        ...
        samples = outputs[:, 0, :, :].to('cpu').numpy()
...
if __name__ == "__main__":
    ...
    # Calculate loss
    with torch.no_grad():
        data_in = masked_topology_sets.get_topology_tensor(as_tensor=True)[0:2000]
        recon_batch = model(data_in)
        data_out = out_topology_sets.get_topology_tensor(as_tensor=True)[0:2000]
