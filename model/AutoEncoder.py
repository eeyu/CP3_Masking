...
@dataclass
class AutoEncoderParameters(ModelParameters):
    ...
    def instantiate_new_model(self):
        return SimpleAutoEncoder(input_channels=self.input_channels,
...
class SimpleAutoEncoder(nn.Module): #Create AE class inheriting from pytorch nn Module class
...
class Encoder(nn.Module): #Encoder model of VAE
    def __init__(self, input_channels, hidden_size, num_layers, latent_dim, image_size, kernel_size, stride):
        ...
        self.fc = nn.Linear(self.final_flatten_size, latent_dim) #Final FC layer to output z
    ...
    def forward(self, x): #Forward call for encoder
        ...
        z = self.fc(x) #Get mu and logvar from FC layers
        return z #Return mu and logvar
...
if __name__ == "__main__":
    ...
    model_parameters = AutoEncoderParameters(input_channels=1,
                                             hidden_size=64,
                                             num_layers=3,
                                             latent_dim=10,
                                             kernel_size=3,
    ...
    for epoch in range(1, num_epochs + 1):
        train(model=model, optimizer=optimizer, batch_size=batch_size, epoch=epoch, data_in_tensor=data_in_tensor,
              data_out_tensor=data_out_tensor)

    save_name = model_parameters.getName() + "CT_" + constraint_type.name + ".pickle"
